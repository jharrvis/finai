...
// --- Configuration ---
import { db, auth, provider, APP_ID } from './config/firebase';
...
// const CATEGORIES removed - using import
...
export default function App() {
    ...
    useEffect(() => {
        localStorage.setItem('showBudgetWidget', String(showBudgetWidget));
    }, [showBudgetWidget]);
    ...
    const handleLogout = async () => {
        try {
            await signOut(auth);
            // Clear all sensitive state immediately
            setTransactions([]);
            setAccounts([]);
            setBudgets([]);
            setMessages([]);
    ...
    // --- Fetch Budgets (Real-time) ---
    useEffect(() => {
        if (!user) return;
        const q = query(
            collection(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets')
        );
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Budget[];
            setBudgets(data);

            // Auto-create default budget for testing if empty
            if (data.length === 0) {
                ensureDefaultBudget();
            }
        });
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            if (!user) return;
            const userCats = await CategoryService.getUserCategories(user.uid);
    ...
    const handleAddCategory = async () => {
        ...
        const updated = await CategoryService.addCategory(user.uid, categories, newCategoryName.trim());
    ...
    const handleDeleteCategory = async (catName: string) => {
        ...
        setConfirmation({
            isOpen: true,
            message: `Hapus kategori "${catName}"? Transaksi lama tetap ada.`,
            onConfirm: async () => {
                const updated = await CategoryService.deleteCategory(user.uid, categories, catName);
                setCategories(updated);
    ...
    const ensureDefaultBudget = async () => {
        if (!user) return;
        // Seed some default budgets
        await addDoc(collection(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets'), {
            category: 'Makan',
            amount: 3000000,
            period: 'monthly',
            userId: user.uid
    ...
    const processAIInput = async (text: string, base64Override: string | null = null) => {
        ...
        const handleAIProcess = async (text: string, imageBase64: string | null) => {
            setIsProcessing(true); // Changed from setIsAIProcessing to setIsProcessing
            try {
                // Use new AIService
                ...
                setIsProcessing(false); // Changed from setIsAIProcessing to setIsProcessing
            }
        };

        await handleAIProcess(text, imageToSend); // Call the new modular function
    ...
    const saveBudget = async () => {
        if (!user || !budgetForm.amount || !budgetForm.category) return;
        ...
        try {
            ...
            if (existingBudget) {
                await updateDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets', existingBudget.id), {
                    amount: Number(budgetForm.amount),
                    period: 'monthly'
                });
                showNotification(`Budget ${budgetForm.category} diperbarui!`, 'success');
            } else {
                await addDoc(collection(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets'), {
                ...
                showNotification(`Budget ${budgetForm.category} dibuat!`, 'success');
            }
            setBudgetForm({});
            setShowBudgetModal(false);
        } catch (error) {
            console.error("Error saving budget:", error);
            showNotification("Gagal menyimpan budget", 'error');
        }
    };
    ...
    const deleteBudget = async (id: string) => {
        ...
        setConfirmation({
            ...
            onConfirm: async () => {
                try {
                    await deleteDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets', id));
                    showNotification("Budget dihapus.", 'success');
                    if (budgetForm.category === budgets.find(b => b.id === id)?.category) {
                        setBudgetForm(prev => ({ ...prev, amount: 0 }));
                    }
                } catch (error) {
                    console.error("Error deleting budget:", error);
                    showNotification("Gagal menghapus budget", 'error');
                }
                setConfirmation(null);
            }
        });
        ...
        <div className="flex flex-col h-screen max-w-md mx-auto bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white font-sans overflow-hidden border-x border-slate-200 dark:border-slate-800 transition-colors duration-300 relative">
            ...
            <main className="flex-1 overflow-y-auto px-6 pb-32 scrollbar-hide">
                                    ...
                                    {showBudgetWidget && (
                                        <div className="bg-white dark:bg-slate-900 p-5 rounded-[24px] border border-slate-100 dark:border-slate-800 shadow-sm">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <p className="text-xs font-bold">Monitor Budget</p>
                                            </div>
                                            <BudgetAlertWidget
                                                budgets={budgets}
                ...
                {activeTab === 'reports' && (
                    <div className="space-y-6 pt-4">
                        {/* AI Insight Card */}
                        <div className="bg-indigo-600 text-white p-6 rounded-[32px] shadow-lg shadow-indigo-200 dark:shadow-none relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-10 -mt-10 blur-2xl"></div>

                            <div className="flex justify-between items-start mb-4 relative z-10">
                                <div>
                                    <h3 className="font-black text-lg flex items-center gap-2"><Bot size={20} /> Analisis AI</h3>
                                    <p className="text-xs text-indigo-200">Insight cerdas untuk keuanganmu</p>
                                </div>
                                {insight && (
                                    <button onClick={() => setInsight(null)} className="p-1 bg-white/20 rounded-full hover:bg-white/30"><X size={14} /></button>
                                )}
                            </div>

                            {insight ? (
                                <div className="prose prose-sm prose-invert max-w-none">
                                    <FormattedMessage content={insight} isUser={false} />
                                </div>
                            ) : (
                                <div className="text-center py-4">
                                    <p className="text-sm text-indigo-100 mb-4">Dapatkan analisis mendalam tentang pengeluaran, saving rate, dan saran penghematan.</p>
                                    <button
                                        onClick={async () => {
                                            setIsAnalyzing(true);
                                            try {
                                        ...
                                        className="bg-white text-indigo-600 px-6 py-3 rounded-xl font-bold text-sm shadow-sm hover:bg-indigo-50 active:scale-95 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2 mx-auto"
                                    >
                                        {isAnalyzing ? (
                                            <>
                ...
                {activeTab === 'settings' && (
                    <div className="space-y-6 pt-4">
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800 space-y-6">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-bold">Widget Budget</p>
                                        <p className="text-xs opacity-50">{showBudgetWidget ? 'Ditampilkan' : 'Disembunyikan'}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setShowBudgetWidget(!showBudgetWidget)}
                                    className={`w-14 h-8 rounded-full p-1 transition-all ${showBudgetWidget ? 'bg-indigo-600' : 'bg-slate-200'}`}
                                >
                                    <div className={`w-6 h-6 bg-white rounded-full shadow-sm transition-all ${showBudgetWidget ? 'translate-x-6' : 'translate-x-0'}`}></div>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-800">
                            <div className="flex justify-between items-center mb-4" onClick={() => setShowCategorySettings(!showCategorySettings)}>
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl">
                                        <ShoppingBag size={24} />
                                    </div>
                                    <div>
                                        <h4 className="font-bold">Kategori Transaksi</h4>
                                        <p className="text-xs text-slate-400">Atur kategori sendiri</p>
                                    </div>
                                </div>
                                <button className="text-slate-400">
                                    {showCategorySettings ? <X size={20} /> : <Settings size={20} />}
                                </button>
                            </div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl space-y-3">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {categories.map((cat, idx) => (
                                            <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-full px-3 py-1 text-xs flex items-center gap-2">
                                                <span>{cat}</span>
                                                <button onClick={() => handleDeleteCategory(cat)} className="text-rose-500 hover:bg-rose-50 rounded-full p-0.5">
                                                    <X size={12} />
                                                </button>
                                            </div>
                                        ))}
            ...
            {
                showBudgetModal && (
                    <div className="fixed inset-0 z-[180] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-200">
                        <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[32px] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold">Atur Budget Bulanan</h3>
                                <button onClick={() => setShowBudgetModal(false)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full"><X size={20} /></button>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={e => {
                                            ...
                                            setBudgetForm({ category: cat, amount: existing?.amount || 0 });
                                        }}
                                    >
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase ml-1">Batas Budget (Rp)</label>
                                    <input
                                        type="number"
                                        className="w-full bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl mt-1 font-bold focus:outline-none"
                                        placeholder="0"
                                ...
                                <button
                                    ...
                                    className="w-full py-4 rounded-2xl bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-200 mt-4 disabled:opacity-50 hover:scale-[1.02] transition-all"
                                >
                                    Simpan Budget
                                </button>

                                <div className="mt-6 border-t border-slate-100 dark:border-slate-800 pt-4">
                                    <p className="text-xs font-bold text-slate-400 mb-3 uppercase">Budget Terpasang</p>
                                    <div className="space-y-2 max-h-40 overflow-y-auto">
                                        {budgets.map(b => (
                                            <div key={b.id} className="flex justify-between items-center text-sm p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
                                                <span className="font-bold">{b.category}</span>
                                                <div className="flex items-center gap-2">
                                                    <span>Rp {formatCurrency(b.amount)}</span>
                                                    <button
                                                        onClick={() => setBudgetForm({ category: b.category, amount: b.amount })}
                                                        className="text-indigo-500 hover:bg-white p-1 rounded"
                                                    >
                                                        <Edit size={14} />
                                                    </button>
                                                    <button
                                                        onClick={() => deleteBudget(b.id)}
                                                        className="text-rose-500 hover:bg-white p-1 rounded"
                                                    >
                                                        <Trash2 size={14} />
                                                    </button>
                                                </div>
                                        ...
                                        {budgets.length === 0 && <p className="text-sm text-slate-400 italic">Belum ada budget diatur.</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
